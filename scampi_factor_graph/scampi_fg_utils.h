...
{
    Eigen::Matrix<T, 3, 1> sx, rot_b, b_in_w;
                             ...
                             CatenaryVariables<T> *cat_vars)
{
    ...
    for(int i=0; i < robotparams_.pulleys.size(); i++)
    {
        ...
        Map<Eigen::Matrix<T, -1, 1>> p(geometric_vars.p_in_w[i].data(),2);
        Map<Eigen::Matrix<T, -1, 1>> b(geometric_vars.b_in_w[i].data(),2);
...
void getCableForces(T fh, T fv,
                      ...
                      GeometricVariables<T> geometric_vars)
{
    Eigen::Matrix<T, -1, -1> structure_matrix(6,8);

    //The wrench applied to the end-effector
    Eigen::Matrix<T, -1, 1> platform_wrench(6);
    ...
    Eigen::Matrix<T, -1, -1> sub_structure_matrix = structure_matrix.block(0,2, 6,6);
    Eigen::Matrix<T, -1, 1> other_forces = sub_structure_matrix.inverse()*(platform_wrench - structure_matrix.block(0,0, 6,2)*f_v.block(0,0,2,1));
    Eigen::Matrix<T, -1, 1> cable_forces(8);
    cable_forces.block(0,0,2,1) = f_v;
    cable_forces.block(2,0,6,1) = other_forces;

    Eigen::Matrix<T, 3, 1> F = structure_matrix.block(0,0,3,8) * cable_forces - Eigen::Matrix<T,3,1>((T)0, (T)0, robotparams_.f_g);
    Eigen::Matrix<T, 3, 1> Torque = structure_matrix.block(3,0,3,8) * cable_forces;
